...
class Generic_WSI_Survival_Dataset(Dataset):
    def __init__(self,
        ...
        metadata = ['case_id','label','disc_label','slide_id','cohort','age','censorship','survival_months','tnm.stage','sex']
            ...
            if self.data_dir:
                if self.mode == 'path':
                    ...
                    for slide_id in slide_ids:
                        wsi_path = os.path.join(data_dir, 'pt_files', '{}.pt'.format(slide_id))
                    ...
                    path_features = torch.cat(path_features, dim=0)
                    genomic_features = torch.tensor(self.genomic_features.iloc[idx])
                    # raw_genomic = torch.tensor(self.raw_genomic.iloc[idx])
                    ...
                    genomic_features = np.array(self.genomic_features.iloc[idx])
                    return (path_features, genomic_features.unsqueeze(dim=0), label, event_time, c)
...
class Generic_Split(Generic_MIL_Survival_Dataset):
    def __init__(self, slide_data, metadata, mode, 
        ...
        self.raw_genomic = self.genomic_features
        ### <--
